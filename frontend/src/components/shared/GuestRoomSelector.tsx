...
import { useBookingSearch, formatBookingSummary } from '../../contexts/BookingSearchContext';
...
const Stepper: React.FC<{
  ...
  <div className="flex items-center gap-2.5">
    ...
    <span className="w-6 text-center text-sm font-extrabold text-[#0B192C] dark:text-white tabular-nums">{value}</span>
    <button
      type="button"
      onClick={onIncrement}
      disabled={value >= max}
...
export const GuestRoomSelector: React.FC<GuestRoomSelectorProps> = ({
  ...
  const handleUpdate = (field: keyof GuestRoomValues, delta: number) => {
    const mins: Record<keyof GuestRoomValues, number> = { rooms: 1, adults: 1, children: 0 };
    const maxes: Record<keyof GuestRoomValues, number> = { rooms: 9, adults: 20, children: 10 };

    const currentMap = { rooms: activeRooms, adults: activeAdults, children: activeChildren };
    const nextVal = Math.max(mins[field], Math.min(maxes[field], currentMap[field] + delta));
  ...
  const summaryLabel = formatBookingSummary(activeRooms, activeAdults, activeChildren);
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              className="absolute left-0 top-full mt-2 w-[280px] bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-2xl shadow-xl z-50 p-5 space-y-4"
            >
              <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-gray-400">Guests & Rooms</h4>

              <div className="flex items-center justify-between">
                <p className="text-xs font-bold text-[#0B192C] dark:text-white">Rooms</p>
                <Stepper
                  value={activeRooms}
                  min={1}
                  max={9}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-bold text-[#0B192C] dark:text-white">Adults</p>
                  <p className="text-[10px] text-gray-400 font-medium">18+ Years</p>
                </div>
                <Stepper
                  value={activeAdults}
                  min={1}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-bold text-[#0B192C] dark:text-white">Children</p>
                  <p className="text-[10px] text-gray-400 font-medium">0–17 Years</p>
                </div>
                <Stepper
  ...
  return (
    <div className="flex-1 min-w-0">
      <label className="block text-[10px] font-extrabold uppercase tracking-wider text-gray-400 mb-1.5 pl-1">Guests & Rooms</label>
      <div className="relative flex items-center" ref={containerRef}>
        ...
        <ChevronDown size={14} className={`absolute right-0 text-gray-400 pointer-events-none transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 6, scale: 0.98 }}
              transition={{ duration: 0.15, ease: 'easeOut' }}
              className="absolute right-0 top-full mt-3 w-[280px] bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-2xl shadow-xl shadow-[#0B192C]/10 z-50 p-5 space-y-5"
            >
              <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-gray-400">Guests & Rooms</h4>
