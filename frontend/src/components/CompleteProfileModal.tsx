...
interface Props {
  ...
  onSubmit: (name: string, phone: string) => Promise<{ success: boolean; message?: string }>;
  onDone: () => void;
  onCancel: () => void;
}
 ...
 */
export const CompleteProfileModal: React.FC<Props> = ({ email, suggestedName = '', onSubmit, onDone, onCancel }) => {
      ...
      <div className="w-full max-w-md bg-white/95 dark:bg-[#0B192C]/95 backdrop-blur-xl border border-white/40 dark:border-slate-800 rounded-[28px] shadow-2xl p-6 sm:p-8 space-y-5">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-gray-400 font-semibold">
            Signing up as <span className="text-[#0A4DA6] font-bold">{email}</span>
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? 'Creating account…' : <>Finish &amp; Continue <ArrowRight size={16} /></>}
          </button>
