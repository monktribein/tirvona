    ...
    loadGis()
      .then(() => {
        ...
        if (!host) {
          ...
          host.style.cssText = 'position:fixed;opacity:0;pointer-events:none;width:0;height:0;overflow:hidden;';
          document.body.appendChild(host);
        }
        host.innerHTML = '';
        google.accounts.id.renderButton(host, { type: 'standard', size: 'large' });

        const realButton = host.querySelector<HTMLElement>('div[role="button"], button');
        if (realButton) {
          realButton.click();
        } else {
