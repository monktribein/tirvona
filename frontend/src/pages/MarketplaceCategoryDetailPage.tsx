...
export const MarketplaceCategoryDetailPage: React.FC = () => {
  ...
  const fetchCategoryDetail = async () => {
    ...
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/marketplace/category/${slug}`
      );
      if (res.data.success) {
  ...
  const handleAddToCart = (product: any) => {
    addNotification('Added to Cart!', `"${product.productName}" added to your sacred cart.`, 'success');
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-slate-950 p-8 flex items-center justify-center">
        <div className="space-y-4 text-center">
          ...
          <p className="text-xs font-extrabold text-gray-400">Loading Sacred Category Landing Page...</p>
        </div>
      </div>
    );
  }
  ...
  if (!categoryData || !categoryData.category) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-slate-950 p-12 text-center space-y-4">
        ...
        <h2 className="text-2xl font-black text-[#0B192C] dark:text-white">Category Not Found</h2>
        <p className="text-xs text-gray-400">This marketplace category might have been renamed or moved.</p>
        <Link to="/marketplace/categories" className="inline-block px-6 py-3 bg-[#0A4DA6] text-white font-bold text-xs rounded-full">
  ...
  const { category, products, trustedSellers, reviews, faqs, relatedCategories } = categoryData;

  const filteredProducts = (products || []).filter((p: any) => {
    let matchesPrice = true;
    if (selectedPrice === 'Under ₹350') matchesPrice = p.price <= 350;
    if (selectedPrice === '₹350 - ₹600') matchesPrice = p.price > 350 && p.price <= 600;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 pb-28 space-y-10">
      ...
      <div className="relative bg-black text-white min-h-[460px] flex items-center overflow-hidden">
        <img
          src={category.bannerImage || category.coverImage || '/banner/ashram_rishikesh.png'}
          alt={category.name}
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-8 py-12 space-y-6 z-10 w-full">
          ...
          <div className="flex items-center gap-2 text-xs font-semibold text-gray-300">
            ...
            <Link to="/marketplace/categories" className="hover:text-white">Marketplace</Link>
            <ChevronRight size={12} />
          ...
          <div className="flex flex-wrap items-center gap-3">
            <span className="px-4 py-1.5 rounded-full bg-amber-500 text-white text-xs font-black uppercase tracking-wider shadow-lg flex items-center gap-1">
              <Flame size={14} /> {category.trendingBadge || 'AUTHENTIC MAHASWET'}
            </span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 text-white max-w-3xl">
            <div className="space-y-0.5">
              <div className="text-[10px] font-bold text-gray-300 uppercase">Famous Temple</div>
              <div className="font-extrabold text-xs sm:text-sm text-amber-400 truncate">{category.templeName}</div>
            </div>

            <div className="space-y-0.5">
              <div className="text-[10px] font-bold text-gray-300 uppercase">Avg Delivery</div>
              <div className="font-black text-xs sm:text-sm flex items-center gap-1">
                <Truck size={14} className="text-emerald-400" /> {category.deliveryDays || 2} Days
              </div>
            </div>

            <div className="space-y-0.5">
              <div className="text-[10px] font-bold text-gray-300 uppercase">Verified Vendors</div>
              <div className="font-black text-xs sm:text-sm flex items-center gap-1">
                <ShieldCheck size={14} className="text-amber-400" /> {category.sellerCount || 15}+ Vendors
              </div>
            </div>

            <div className="space-y-0.5">
              <div className="text-[10px] font-bold text-gray-300 uppercase">Customer Rating</div>
              <div className="font-black text-xs sm:text-sm flex items-center gap-1">
                <Star size={14} className="text-amber-400 fill-amber-400" /> {category.rating || 4.9} / 5.0
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs text-gray-600 dark:text-gray-300 font-semibold leading-relaxed">
            <div className="space-y-3 bg-gray-50 dark:bg-slate-900/60 p-5 rounded-2xl border border-gray-100 dark:border-slate-800">
              <h3 className="font-extrabold text-sm text-[#0B192C] dark:text-white flex items-center gap-1.5">
                <BookOpen size={15} className="text-[#0A4DA6]" /> Origin & Temple History
              </h3>
              <p>{category.history || `${category.name} has been prepared at ${category.templeName} for centuries following authentic Vedic recipes using pure cow ghee.`}</p>
            </div>

            <div className="space-y-3 bg-gray-50 dark:bg-slate-900/60 p-5 rounded-2xl border border-gray-100 dark:border-slate-800">
              <h3 className="font-extrabold text-sm text-[#0B192C] dark:text-white flex items-center gap-1.5">
                <Award size={15} className="text-amber-500" /> Why Devotees Revere It
              </h3>
              <p>{category.importance || category.whyFamous || `Crafted by traditional halwais and offered directly as Mahaprasad to the deity before packaging.`}</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-gray-400 font-semibold">Packed directly from certified temple vendors in {category.originCity}.</p>
            </div>
            ...
            <div className="flex items-center gap-2 overflow-x-auto w-full sm:w-auto pb-1 sm:pb-0">
              ...
              <button
                ...
                className={`px-3.5 py-1.5 rounded-full text-xs font-black cursor-pointer whitespace-nowrap transition-all ${
                  filterOrganic ? 'bg-emerald-600 text-white' : 'bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-gray-300'
                }`}
              >
                100% Organic
              </button>
              ...
              >
                <div>
                  <div className="relative aspect-video bg-black overflow-hidden">
                    <img
                      src={prod.images?.[0] || category.coverImage || '/banner/ashram_rishikesh.png'}
                  ...
                  <div className="p-6 space-y-3">
                    ...
                    <div className="flex items-center justify-between text-xs font-bold text-gray-400 pt-1">
                      <span className="flex items-center gap-1 text-[#0A4DA6] dark:text-amber-400">
                        <ShieldCheck size={13} /> {prod.storeName || category.templeName}
                      </span>
                      <span className="flex items-center gap-1 text-amber-500">
                        <Star size={12} className="fill-amber-500" /> {prod.rating || 4.9}
                      </span>
                    </div>
                  </div>
                </div>
        ...
        {trustedSellers && trustedSellers.length > 0 && (
          <div className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm">
            <h2 className="text-xl font-black text-[#0B192C] dark:text-white flex items-center gap-2">
              <ShieldCheck size={20} className="text-[#0A4DA6]" /> Verified & Certified Temple Vendors
            </h2>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {trustedSellers.map((seller: any) => (
                <div key={seller.id} className="bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-4 flex items-center gap-4">
                  <img src={seller.photo} alt={seller.name} className="w-14 h-14 rounded-2xl object-cover shrink-0" />
                  <div className="space-y-1">
                    <div className="font-extrabold text-sm text-[#0B192C] dark:text-white flex items-center gap-1.5">
                      <span>{seller.name}</span>
                      <ShieldCheck size={14} className="text-emerald-500" />
                    </div>
                    <p className="text-xs text-gray-500 font-semibold">{seller.templeAssociation} • {seller.yearsOfService} Yrs Service</p>
                    <div className="text-[11px] font-black text-amber-500 flex items-center gap-1">
                      <Star size={11} className="fill-amber-500" /> {seller.rating} Rating
                    </div>
                  </div>
                </div>
              ))}
        ...
        {faqs && faqs.length > 0 && (
          <div className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-3xl p-6 sm:p-8 space-y-4 shadow-sm">
            <h2 className="text-xl font-black text-[#0B192C] dark:text-white flex items-center gap-2">
              <HelpCircle size={20} className="text-gray-400" /> Frequently Asked Questions
            </h2>

            <div className="space-y-3">
              {faqs.map((faq: any, idx: number) => (
                <div key={idx} className="bg-gray-50 dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl p-4 space-y-1">
                  <h4 className="font-extrabold text-sm text-[#0B192C] dark:text-white flex items-center gap-2">
                    <span className="text-[#0A4DA6] font-black">Q:</span> {faq.q}
                  </h4>
                  <p className="text-xs text-gray-500 dark:text-gray-400 font-semibold pl-5 leading-relaxed">{faq.a}</p>
                </div>
              ))}
            </div>
          </div>
