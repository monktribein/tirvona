...
const BANNER_SIZE_PRESETS = [
  { id: '1920x600', label: 'Desktop Hero Banner', width: 1920, height: 600, ratio: '16:5' },
  { id: '800x600', label: 'Mobile Hero Banner', width: 800, height: 600, ratio: '4:3' },
  { id: '1200x400', label: 'Festival Special Banner', width: 1200, height: 400, ratio: '3:1' },
  { id: '1000x500', label: 'Offer Directory Banner', width: 1000, height: 500, ratio: '2:1' },
  { id: '600x600', label: 'Square Marketplace Banner', width: 600, height: 600, ratio: '1:1' },
  { id: 'custom', label: 'Custom Specification', width: 1920, height: 600, ratio: 'Custom' },
];
...
export const BannerBoyDashboard: React.FC = () => {
  ...
  const handleFileUpload = async (file: File) => {
    if (!file.type.startsWith('image/')) {
      addNotification('Invalid File Type', 'Please upload an image file (JPG, PNG, WEBP, GIF).', 'error');
      return;
    }
    ...
    try {
      ...
      addNotification('Upload Failed', getErrorMessage(err, 'Failed to upload image from computer.'), 'error');
    } finally {
  ...
  const handleSubmitRequest = async (e: React.FormEvent) => {
    ...
    if (!bannerImageUrl) {
      addNotification('Missing Image', 'Please upload or provide a banner image URL.', 'warning');
      return;
    }

    setSubmitting(true);
    try {
      const activePreset = BANNER_SIZE_PRESETS.find((p) => p.id === selectedPreset);
      ...
      if (err?.response?.status === 401) {
        addNotification(
          'Session Expired (401)',
          'Your authentication session has expired. Please Sign Out and log in again as bannerboy@tirvona.com (admin123).',
          'error'
        );
      } else {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 p-6 rounded-[24px] shadow-sm">
        <div>
          ...
          <p className="text-xs text-gray-500 font-medium mt-0.5">
            Enterprise CMS Portal — Upload banners, set pixel dimensions, and manage homepage media.
          </p>
        </div>
        <a
          href="/"
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
            <div className="p-5 bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-[24px] shadow-lg shadow-gray-200/40 dark:shadow-none hover:shadow-xl transition-all space-y-1">
              ...
              <h3 className="text-2xl font-black text-[#0B192C] dark:text-white mt-1">{myRequests.length} Submissions</h3>
            </div>
            <div className="p-5 bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-[24px] shadow-lg shadow-gray-200/40 dark:shadow-none hover:shadow-xl transition-all space-y-1">
              <span className="text-[10px] font-black text-amber-500 uppercase tracking-wider block">Pending Approvals</span>
              <h3 className="text-2xl font-black text-amber-600 mt-1">{pendingCount} Pending</h3>
            </div>
            <div className="p-5 bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-[24px] shadow-lg shadow-gray-200/40 dark:shadow-none hover:shadow-xl transition-all space-y-1">
              <span className="text-[10px] font-black text-emerald-500 uppercase tracking-wider block">Approved & Live</span>
              <h3 className="text-2xl font-black text-emerald-600 mt-1">{approvedCount} Published</h3>
            </div>
            <div className="p-5 bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-[24px] shadow-lg shadow-gray-200/40 dark:shadow-none hover:shadow-xl transition-all space-y-1">
              <span className="text-[10px] font-black text-rose-500 uppercase tracking-wider block">Rejected Changes</span>
              <h3 className="text-2xl font-black text-rose-600 mt-1">{rejectedCount} Rejected</h3>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 p-6 rounded-[24px] shadow-sm space-y-6">
            ...
            <form onSubmit={handleSubmitRequest} className="space-y-6 text-xs">
              ...
              <div className="p-5 bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4">
                <div className="flex justify-between items-center">
                  <span className="font-extrabold text-xs text-[#0B192C] dark:text-white flex items-center gap-2">
                    <Maximize2 size={16} className="text-[#0A4DA6]" /> Banner Size & Aspect Ratio Specifications
                  </span>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {BANNER_SIZE_PRESETS.map((preset) => (
                    ...
                    >
                      <span className="font-extrabold text-[11px] block truncate">{preset.label}</span>
              ...
              <div className="space-y-2">
                <label className="font-bold text-gray-700 dark:text-gray-300 flex items-center justify-between">
                  <span className="flex items-center gap-1.5">
                    <Upload size={16} className="text-[#0A4DA6]" /> Upload Banner Image File
                  </span>
                  <span className="text-[10px] text-gray-400 font-normal">Supports JPG, PNG, WEBP, GIF (Max 10MB)</span>
                </label>
                ...
                >
                  {uploadingImage ? (
                    <div className="py-4 space-y-2 flex flex-col items-center">
                      <RefreshCw size={28} className="animate-spin text-[#0A4DA6]" />
                      <span className="font-extrabold text-xs text-[#0A4DA6]">Uploading banner file to server...</span>
                    </div>
                  ) : (
                    <div className="space-y-3 flex flex-col items-center">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-[11px] text-gray-400">
                          Recommended format: JPG/WEBP optimized for {bannerWidth} × {bannerHeight} px
                        </p>
                      </div>
                      <button
                        type="button"
                ...
                {bannerImageUrl && (
                  <div className="p-4 bg-white dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-2xl flex flex-col sm:flex-row items-center gap-4 shadow-sm mt-3">
                    ...
                    <div className="flex-1 space-y-1.5 text-xs min-w-0">
                      ...
                      <p className="text-[11px] text-gray-400 font-mono truncate">{bannerImageUrl}</p>
                      <div className="flex items-center gap-3 text-[11px] text-gray-500 pt-1">
                        <span>Preset: <strong>{BANNER_SIZE_PRESETS.find(p => p.id === selectedPreset)?.label}</strong></span>
                        <span>•</span>
                        <span>Dimensions: <strong>{bannerWidth} × {bannerHeight} px</strong></span>
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="font-bold text-gray-700 dark:text-gray-300">Top Bar Announcement</label>
                  <input
                    type="text"
