    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 min-h-[calc(100vh-12rem)] text-left">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2 overflow-y-auto max-h-[400px]">
            {loading ? (
              ...
              <div className="text-xs text-gray-400 text-center py-8">No tickets logged</div>
            ) : (
              tickets.map((t) => (
                <div
                  key={t._id}
                ...
                >
                  <div className="flex justify-between items-start gap-2">
                    <span className="text-xs font-extrabold truncate max-w-[140px] text-[#0B192C] dark:text-white">{t.title}</span>
                    <span className={`px-2 py-0.5 rounded-full text-[8px] font-bold capitalize ${
                      t.status === 'resolved' ? 'bg-success/10 text-success' : 'bg-yellow-50 text-yellow-750'
                    }`}>
                      {t.status}
                    </span>
                  </div>
        ...
        {activeTicket ? (
          <>
            ...
            <div className="p-4 border-b border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-900/10 flex justify-between items-center">
              <div>
                <h4 className="font-extrabold text-sm text-[#0B192C] dark:text-white">{activeTicket.title}</h4>
                <p className="text-[10px] text-gray-450 mt-0.5 font-semibold">{activeTicket.description}</p>
              </div>
              ...
              {activeTicket.messages?.map((msg: any, i: number) => {
                ...
                return (
                  <div key={i} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`p-3 px-4 rounded-[20px] max-w-sm text-xs leading-relaxed ${
                      isMe 
            ...
            {activeTicket.status !== 'resolved' && (
              <form onSubmit={handleSendMessage} className="p-4 border-t border-gray-100 dark:border-slate-800 flex gap-2">
                ...
                />
                <button type="submit" className="p-2.5 bg-[#0A4DA6] text-white rounded-xl cursor-pointer hover:bg-opacity-95 transition-all">
                  <Send size={14} />
                </button>
              </form>
            )}
          ...
          <div className="flex flex-col items-center justify-center text-center p-12 flex-grow space-y-2">
            ...
            <h4 className="font-extrabold text-sm text-[#0B192C] dark:text-white">Select a ticket to begin</h4>
            <p className="text-xs text-gray-400">Pick any open ticket from the sidebar to chat with support.</p>
          </div>
        )}
      </section>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <form onSubmit={handleCreateTicket} className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 max-w-md w-full rounded-[28px] p-6 space-y-4">
            <div className="flex justify-between items-center border-b border-gray-100 dark:border-slate-800 pb-3">
              <h3 className="font-bold text-sm text-[#0B192C] dark:text-white">File a Support Request</h3>
              <button type="button" onClick={() => setShowCreate(false)} className="text-gray-400 hover:text-gray-655">
                <X size={18} />
              </button>
            </div>

            <div className="space-y-1">
              <label className="text-xs font-bold text-gray-400">Subject Title</label>
              <input
                type="text"
                required
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-gray-400">Describe Issue</label>
              <textarea
                required
                rows={3}
