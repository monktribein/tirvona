...
import { MapPin, Clock, ArrowLeft, ShieldCheck, CheckCircle2, Phone, Globe, Info, AlertTriangle } from 'lucide-react';
...
export const TempleDetailPage: React.FC = () => {
  ...
  const fetchTempleDetail = async () => {
    ...
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/services/temples/${slug}`
      );
      if (res.data.success) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-[#070F1B] pt-32 text-center">
        ...
        <p className="text-xs font-bold text-gray-500">Loading Temple Profile...</p>
      </div>
    );
  }
  ...
  if (!temple) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-[#070F1B] pt-32 text-center">
        <h2 className="text-xl font-black text-gray-700 dark:text-gray-200 mb-4">Temple Profile Not Found</h2>
        <button onClick={() => navigate('/temples')} className="px-6 py-2.5 rounded-full bg-[#0A4DA6] text-white font-bold text-xs">
          Back to All Temples
        </button>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#070F1B] pt-24 sm:pt-28 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 space-y-8">
        ...
        <div className="relative rounded-3xl overflow-hidden shadow-2xl min-h-[320px] sm:min-h-[400px] flex items-end p-6 sm:p-10 border border-gray-100 dark:border-slate-800">
          <img src={temple.coverImage} alt={temple.name} className="absolute inset-0 w-full h-full object-cover" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white dark:bg-[#0B192C] rounded-3xl p-6 sm:p-8 border border-gray-100 dark:border-slate-800 space-y-4">
              <h3 className="font-black text-xl text-[#0B192C] dark:text-white">Temple History & Sacred Significance</h3>
              <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 leading-relaxed">
                {temple.history}
              </p>
              <div className="pt-2 text-xs font-bold text-gray-500">
                Architecture Style: <span className="text-[#0B192C] dark:text-white">{temple.architectureStyle}</span>
              </div>
            </div>

            {/* Timings & Dress Code */}
            <div className="bg-white dark:bg-[#0B192C] rounded-3xl p-6 sm:p-8 border border-gray-100 dark:border-slate-800 space-y-6">
              <h3 className="font-black text-xl text-[#0B192C] dark:text-white">Darshan & Aarti Schedule</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl bg-blue-50/50 dark:bg-slate-900 border border-blue-100 dark:border-slate-800 space-y-1">
                  <span className="text-[10px] text-gray-400 font-extrabold uppercase block">Darshan Timings</span>
                  <p className="font-black text-sm text-[#0A4DA6] dark:text-amber-400">{temple.darshanTimings}</p>
                </div>
                <div className="p-4 rounded-2xl bg-blue-50/50 dark:bg-slate-900 border border-blue-100 dark:border-slate-800 space-y-1">
                  <span className="text-[10px] text-gray-400 font-extrabold uppercase block">Aarti Schedule</span>
                  <p className="font-black text-sm text-[#0A4DA6] dark:text-amber-400">{temple.aartiTimings}</p>
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-amber-50/80 dark:bg-slate-900/80 border border-amber-200/50 dark:border-slate-800 space-y-2">
                ...
                <p className="text-xs text-gray-700 dark:text-gray-300 font-medium">{temple.dressCode}</p>
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-[#0B192C] rounded-3xl p-6 border border-gray-100 dark:border-slate-800 space-y-4 shadow-sm">
              <h4 className="font-black text-base text-[#0B192C] dark:text-white">Temple Contact & Trust</h4>
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="flex items-center gap-2 text-gray-600 dark:text-gray-300">
                  ...
                  <a href={temple.officialWebsite} target="_blank" rel="noopener noreferrer" className="text-[#0A4DA6] hover:underline font-bold">
                    Official Website
                  </a>
                </div>
              </div>
            ...
            <div className="bg-slate-900 text-white rounded-3xl p-6 space-y-3 border border-slate-800">
              ...
              <h5 className="font-black text-sm">Live Darshan, VIP Queue & Online Seva</h5>
              <p className="text-[11px] text-gray-400">Direct online token booking for VIP darshan and daily prasad dispatch from trust kitchen.</p>
            </div>
          </div>
