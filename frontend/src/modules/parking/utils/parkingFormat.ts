import type { ParkingBookingStatus, ParkingVehicleTypeCode } from '../types/parking.types';

// Display helpers for the Parking module.

/** Indian rupee formatting, matching how prices read elsewhere on the site. */
export const formatCurrency = (amount: number | undefined | null) => `₹${Number(amount || 0).toLocaleString('en-IN')}`;
...
export const vehicleLabel = (code?: ParkingVehicleTypeCode | string) =>
  (code && VEHICLE_LABELS[code as ParkingVehicleTypeCode]) || String(code || '').replace(/_/g, ' ');
...
export const statusLabel = (status: ParkingBookingStatus) => STATUS_LABELS[status] || status;
...
export const STATUS_STYLES: Record<ParkingBookingStatus, string> = {
  ...
  upcoming: 'bg-blue-50 dark:bg-blue-950/60 text-[#0A4DA6] dark:text-blue-300 border-blue-200 dark:border-blue-900/50',
  checked_in:
  ...
  checked_out: 'bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-slate-700',
  cancelled: 'bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border-rose-200 dark:border-rose-900/50',
  expired: 'bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-slate-700',
  no_show:
...
export const availabilityTone = (available: number, total: number) => {
  ...
  if (total > 0 && available / total < 0.15) return 'text-amber-600 dark:text-amber-400';
