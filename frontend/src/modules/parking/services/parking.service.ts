...
export const parkingDiscoveryService = {
  search: (
    filters: ParkingSearchFilters & { page?: number; limit?: number } = {},
  ) =>
    api.get("/parking/locations", {
  ...
  getAvailability: (
    locationId: string,
    params: { entryAt: string; exitAt: string; vehicleType?: string },
  ) =>
    api.get(`/parking/locations/${locationId}/availability`, {
      params: clean(params),
    }),

  getReviews: (locationId: string, page = 1, limit = 10) =>
    api.get(`/parking/locations/${locationId}/reviews`, {
      params: { page, limit },
    }),
...
export const parkingBookingService = {
  ...
  cancel: (id: string, reason?: string) =>
    api.post(`/parking/bookings/${id}/cancel`, { reason }),
...
export const parkingScanService = {
  ...
  verify: (token: string, locationId: string) =>
    api.post("/parking/scan/verify", { token, locationId }),

  checkIn: (token: string, locationId: string) =>
    api.post("/parking/scan/check-in", { token, locationId }),

  checkOut: (
    token: string,
    locationId: string,
    overstayPaymentMethod = "cash",
  ) =>
    api.post("/parking/scan/check-out", {
      token,
      locationId,
      overstayPaymentMethod,
    }),
...
export const parkingPartnerService = {
  ...
  createLocation: (payload: unknown) =>
    api.post("/parking/partner/locations", payload),
  updateLocation: (id: string, payload: unknown) =>
    api.put(`/parking/partner/locations/${id}`, payload),

  listSlotTypes: (locationId: string) =>
    api.get(`/parking/partner/locations/${locationId}/slot-types`),
  createSlotType: (locationId: string, payload: unknown) =>
    api.post(`/parking/partner/locations/${locationId}/slot-types`, payload),
  updateSlotType: (id: string, payload: unknown) =>
    api.put(`/parking/partner/slot-types/${id}`, payload),

  listSlots: (
    locationId: string,
    params: { status?: string; slotTypeId?: string } = {},
  ) =>
    api.get(`/parking/partner/locations/${locationId}/slots`, {
      params: clean(params),
    }),
  bulkCreateSlots: (locationId: string, payload: unknown) =>
    api.post(`/parking/partner/locations/${locationId}/slots/bulk`, payload),
  updateSlot: (id: string, payload: unknown) =>
    api.patch(`/parking/partner/slots/${id}`, payload),

  listPricing: (locationId: string) =>
    api.get(`/parking/partner/locations/${locationId}/pricing`),
  ...
  calendar: (
    ...
    api.get(`/parking/partner/locations/${locationId}/calendar`, {
      params: clean(params),
    }),
  ...
  getSettings: (locationId: string) =>
    api.get(`/parking/partner/locations/${locationId}/settings`),
...
export const parkingAdminService = {
  ...
  createPartner: (payload: unknown) =>
    api.post("/parking/admin/partners", payload),
  updatePartnerStatus: (id: string, payload: unknown) =>
    api.patch(`/parking/admin/partners/${id}/status`, payload),

  listLocations: (params: Record<string, unknown> = {}) =>
    api.get("/parking/admin/locations", { params: clean(params) }),
  updateLocationStatus: (id: string, payload: unknown) =>
    api.patch(`/parking/admin/locations/${id}/status`, payload),

  listBookings: (params: Record<string, unknown> = {}) =>
    api.get("/parking/admin/bookings", { params: clean(params) }),
  refundBooking: (id: string, reason?: string) =>
    api.post(`/parking/admin/bookings/${id}/refund`, { reason }),
  ...
  updateSettings: (payload: unknown) =>
    api.put("/parking/admin/settings", payload),

  listHolidays: () => api.get("/parking/admin/holidays"),
  createHoliday: (payload: unknown) =>
    api.post("/parking/admin/holidays", payload),
