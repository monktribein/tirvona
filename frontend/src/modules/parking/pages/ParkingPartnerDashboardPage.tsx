...
import type { ParkingBooking, ParkingDashboardStats, ParkingLocation } from '../types/parking.types';
...
interface ReportBundle {
  revenue?: { totals: { revenue: number; commission: number; partnerEarning: number; bookings: number } };
  peakHours?: { hour: number; label: string; count: number }[];
  cancellations?: { cancellationRate: number; noShowRate: number; total: number };
...
export const ParkingPartnerDashboardPage: React.FC = () => {
  ...
  const [locations, setLocations] = useState<(ParkingLocation & { liveOccupancy?: Record<string, number> })[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [dashRes, locRes, bookRes, repRes] = await Promise.all([
        ...
        parkingPartnerService.listBookings({ status: statusFilter || undefined, limit: 25 }),
      ...
      <div className="max-w-7xl mx-auto px-4 pt-8 pb-20 space-y-4">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-24 bg-gray-100 dark:bg-slate-800 animate-pulse rounded-[24px]" />
          ))}
        </div>
      ...
      {stats && (
        ...
        >
          ...
          <ParkingStatTile
            ...
            tone={stats.occupancyPercent > 85 ? 'danger' : stats.occupancyPercent > 60 ? 'warning' : 'success'}
          />
        </motion.section>
      )}
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <section className="lg:col-span-2 bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-[24px] p-5 space-y-4 shadow-sm">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <h2 className="font-extrabold text-sm text-[#0B192C] dark:text-white">Bookings</h2>
          ...
          {bookings.length === 0 ? (
            <p className="text-xs text-gray-400 font-medium py-8 text-center">No bookings to show.</p>
          ) : (
            <div className="overflow-x-auto -mx-5 px-5">
              <table className="w-full min-w-[36rem]">
                <thead>
                  <tr className="">
                    {['Vehicle', 'Entry', 'Exit', 'Bay', 'Amount', 'Status'].map((h) => (
                      <th
                        key={h}
                  ...
                  {bookings.map((b) => (
                    <tr key={b._id} className="">
                      <td className="py-2.5 pr-3">
                        <p className="text-[11px] font-black text-[#0B192C] dark:text-white">{b.vehicleNumber}</p>
                        <p className="text-[9px] font-semibold text-gray-400">{vehicleLabel(b.vehicleType)}</p>
                      </td>
        ...
        <div className="space-y-5">
          ...
          <section className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-[24px] p-5 space-y-3 shadow-sm">
            ...
            {locations.length === 0 ? (
              <p className="text-xs text-gray-400 font-medium py-2">No parking locations yet.</p>
            ) : (
              <ul className="space-y-2">
                {locations.map((loc) => (
                  <li key={loc._id} className="bg-gray-50 dark:bg-slate-900/60 rounded-2xl p-3 space-y-1.5">
                    ...
                    <p className="text-[9px] font-semibold text-gray-400">{loc.address?.city}</p>
                    {loc.liveOccupancy && (
                      <div className="flex items-center gap-2 pt-0.5">
                        <div className="flex-1 h-1.5 bg-gray-200 dark:bg-slate-800 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: `${Math.min(100, Number(loc.liveOccupancy.occupancyPercent) || 0)}%` }}
                          />
                        </div>
          ...
          {reports.peakHours && reports.peakHours.length > 0 && (
            <section className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-[24px] p-5 space-y-3 shadow-sm">
              ...
              <div className="flex items-end gap-0.5 h-24" role="img" aria-label="Bookings by hour of entry">
          ...
          {reports.revenue && (
            <section className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-[24px] p-5 space-y-2.5 shadow-sm">
              <h2 className="font-extrabold text-sm text-[#0B192C] dark:text-white">Last 30 Days</h2>

              {[
                ['Gross revenue', formatCurrency(reports.revenue.totals.revenue)],
                ['Your earnings', formatCurrency(reports.revenue.totals.partnerEarning)],
                ['Platform commission', formatCurrency(reports.revenue.totals.commission)],
                ['Bookings', String(reports.revenue.totals.bookings)],
                ['Avg. stay', reports.averageStay ? `${reports.averageStay.averageHours} hr` : '—'],
                ['Cancellation rate', reports.cancellations ? `${reports.cancellations.cancellationRate}%` : '—'],
                ['No-show rate', reports.cancellations ? `${reports.cancellations.noShowRate}%` : '—'],
              ].map(([label, value]) => (
                <div key={label} className="flex justify-between text-[11px]">
                  <span className="font-semibold text-gray-500 dark:text-gray-400">{label}</span>
                  <span className="font-black text-[#0B192C] dark:text-white">{value}</span>
                </div>
              ))}
            </section>
          )}
