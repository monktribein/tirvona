...
import type { ParkingBooking, ParkingLocation, ParkingPass, ParkingRefundQuote } from '../types/parking.types';
...
export const ParkingBookingDetailPage: React.FC = () => {
  ...
  const loadQr = useCallback(async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        setPass((prev) => ({ ...(prev || ({} as ParkingPass)), ...res.data.data }));
      }
    } catch {
  ...
  const handleCancel = async () => {
    ...
    setCancelling(true);
    try {
      const res = await parkingBookingService.cancel(id, 'Cancelled by visitor');
      if (res.data?.success) {
  ...
  if (!booking) {
    return (
      <div className="max-w-3xl mx-auto px-4 pt-16 pb-20 text-center space-y-4">
        <AlertCircle size={40} className="text-gray-300 dark:text-slate-700 mx-auto" />
        <h1 className="font-extrabold text-lg text-[#0B192C] dark:text-white">Booking not found</h1>
        <p className="text-xs text-gray-400 font-medium">{error || 'This booking is not available.'}</p>
        <Link
    ...
    <div className="pb-16 lg:pb-24 pt-8 sm:pt-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
        ...
        {justBooked && booking.paymentStatus === 'paid' && (
          ...
          >
            <CheckCircle2 size={20} className="text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5 stroke-[2.5]" />
            <div className="space-y-0.5">
              ...
              <p className="text-[11px] font-semibold text-emerald-700 dark:text-emerald-300/90 leading-relaxed">
                Booking {booking.bookingReference}. Show the QR pass below at the gate for entry and exit.
              </p>
            </div>
          </motion.div>
        )}
        ...
        <div className="grid lg:grid-cols-5 gap-5">
          ...
          <div className="lg:col-span-2 order-1">
            {booking.paymentStatus === 'paid' && !['cancelled', 'expired', 'no_show'].includes(booking.status) ? (
              qrLoading && !pass?.image ? (
                <div className="h-[32rem] bg-gray-100 dark:bg-slate-800 animate-pulse rounded-[28px]" />
              ) : pass ? (
                <ParkingQrTicket booking={booking} pass={pass} locationName={location?.name} />
              ) : null
            ) : (
              <div className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-[28px] p-8 text-center space-y-2 shadow-sm">
                <XCircle size={32} className="text-gray-300 dark:text-slate-700 mx-auto" />
          ...
          <div className="lg:col-span-3 order-2 space-y-4">
            <section className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-[24px] p-5 space-y-4 shadow-sm">
                ...
                <div className="min-w-0 space-y-1">
                  ...
                  {location?.address && (
                    <p className="inline-flex items-center gap-1.5 text-[11px] text-gray-500 dark:text-gray-400 font-medium">
                      ...
                      {[location.address.line1, location.address.city].filter(Boolean).join(', ')}
                    </p>
                  )}
                </div>
              ...
              <dl className="grid grid-cols-2 gap-x-4 gap-y-3 pt-1">
                {[
                  ...
                  ['Vehicle', `${booking.vehicleNumber} · ${vehicleLabel(booking.vehicleType)}`],
                  ...
                  ['Actual stay', formatDuration(booking.actualDurationMinutes)],
                ].map(([label, value]) => (
                  <div key={label} className="pt-2 space-y-0.5">
                    <dt className="text-[9px] uppercase tracking-wider font-bold text-gray-400">{label}</dt>
                    <dd className="text-[11px] font-bold text-slate-700 dark:text-gray-200 break-words">{value}</dd>
                  </div>
                ))}
              </dl>
            </section>
            ...
            <section className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-[24px] p-5 space-y-2.5 shadow-sm">
              <h2 className="font-extrabold text-sm text-[#0B192C] dark:text-white">Payment</h2>

              {[
                ['Parking charges', booking.pricing.durationAmount],
                ['Base fee', booking.pricing.baseFee],
                [`GST (${booking.pricing.taxPercent}%)`, booking.pricing.taxAmount],
                ...(booking.pricing.overstayAmount > 0
                  ? [['Overstay', booking.pricing.overstayAmount] as [string, number]]
                  : []),
                ...(booking.pricing.refundAmount > 0
                  ? [['Refunded', -booking.pricing.refundAmount] as [string, number]]
                  : []),
              ]
                .filter(([, v]) => v !== 0)
                .map(([label, value]) => (
              ...
              <div className="flex justify-between items-center pt-2.5">
                <span className="text-xs font-black text-[#0B192C] dark:text-white">Paid</span>
            ...
            {showCancel && (
              ...
              >
                <h3 className="font-extrabold text-sm text-rose-900 dark:text-rose-200">Cancel this booking?</h3>
                  ...
                  <p className="text-[11px] font-semibold text-rose-700 dark:text-rose-300/90 leading-relaxed">
                    ...
                    {refund.freeCancellationHours !== undefined && (
                      <span className="block mt-1 text-rose-600/80 dark:text-rose-400/70">
                        Full refund applies up to {refund.freeCancellationHours} hours before entry.
                      </span>
                    )}
                  </p>
                ) : (
                ...
                <div className="flex gap-2.5">
                  ...
                  >
                    {cancelling && <Loader2 size={13} className="animate-spin stroke-[2.5]" />}
                    Yes, cancel
                  </button>
                  <button
                    type="button"
            ...
            {canReview && (
              ...
              >
                <h2 className="font-extrabold text-sm text-[#0B192C] dark:text-white">Rate this parking</h2>
                ...
                <div className="flex gap-1.5">
                  {[1, 2, 3, 4, 5].map((n) => (
                    ...
                    >
                      <Star
                        size={24}
                        className={n <= rating ? 'fill-[#D4AF37] text-[#D4AF37]' : 'text-gray-300 dark:text-slate-700'}
                      />
                    </button>
                  ))}
                </div>
