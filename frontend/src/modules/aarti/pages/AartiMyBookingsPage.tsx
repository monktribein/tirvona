...
export const AartiMyBookingsPage: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const response = await aartiBookingService.list({
        status: status || undefined,
        limit: 50,
      });
      setBookings(response.data?.data ?? []);
    } catch (err) {
      setError(getErrorMessage(err, "We could not load your aarti bookings."));
      setBookings([]);
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6">
        <div className="text-center space-y-2 max-w-3xl mx-auto py-2">
          ...
          <p className="text-xs sm:text-sm font-bold text-[#0B192C] dark:text-gray-200 max-w-xl mx-auto leading-relaxed">
            Your passes, gate codes and refund status in one place.
          </p>
        </div>
      </div>
                  ...
                  <div className="min-w-0 flex-1 space-y-1.5">
                    ...
                    <div className="flex flex-wrap gap-4 text-[11px] font-medium text-gray-500 dark:text-gray-400 pt-0.5">
                      ...
                      <span className="inline-flex items-center gap-1.5">
                        ...
                        {booking.passCount} pass
                        {booking.passCount === 1 ? "" : "es"}
                      </span>
                    </div>
                  </div>
...
export default AartiMyBookingsPage;
