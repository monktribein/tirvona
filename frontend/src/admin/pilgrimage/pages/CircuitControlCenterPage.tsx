...
import type {
  PilgrimageCircuit,
  PilgrimageDashboard,
} from "../../../modules/pilgrimage/types/pilgrimage.types";
import {
  circuitStatusLabel,
  circuitStatusStyle,
  difficultyLabel,
...
export const CircuitControlCenterPage: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const [dashboardRes, circuitRes] = await Promise.all([
        pilgrimageAdminService.dashboard(),
        pilgrimageAdminService.listCircuits({
          status: statusFilter || undefined,
          limit: 100,
        }),
      ]);
      setDashboard(dashboardRes.data?.data ?? null);
      setCircuits(circuitRes.data?.data ?? []);
    } catch (err) {
      setError(getErrorMessage(err, "We could not load the pilgrimage console."));
    } finally {
      setLoading(false);
    }
  }, [statusFilter]);
  ...
  const setStatus = async (circuit: PilgrimageCircuit, status: string) => {
    await pilgrimageAdminService
      .setStatus(circuit._id, status)
      .catch(() => undefined);
    await load();
  };

  const toggleFeatured = async (circuit: PilgrimageCircuit) => {
    await pilgrimageAdminService
      .setFeatured(circuit._id, !circuit.isFeatured)
      .catch(() => undefined);
    await load();
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {dashboard?.topCircuits?.length ? (
        <div className={`${CARD} p-5`}>
          <h2 className="flex items-center gap-2 text-base font-black text-[#0B192C] dark:text-white">
            <Eye size={17} className="text-[#0A4DA6] stroke-[2.5]" />
            Most viewed circuits
          </h2>
          <div className="mt-3 space-y-2">
            {dashboard.topCircuits.map((row, index) => (
              <div
                key={row._id}
                ...
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] font-bold text-gray-400">
                      {row.startCity ?? "—"} · {formatDuration(row.durationDays)}{" "}
                      · {row.stopCount ?? 0} stops
                    </p>
                  </div>
                </div>
                  ...
                  <tr key={circuit._id}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          type="button"
                          title={circuit.isFeatured ? "Unfeature" : "Feature"}
                          onClick={() => toggleFeatured(circuit)}
                        ...
                        {circuit.status === "approved" ? (
                          <button
                            type="button"
                            title="Suspend"
                            onClick={() => setStatus(circuit, "suspended")}
                        ...
                        ) : circuit.status === "suspended" ? (
                          <button
                            type="button"
                            title="Restore"
                            onClick={() => setStatus(circuit, "approved")}
...
export default CircuitControlCenterPage;
