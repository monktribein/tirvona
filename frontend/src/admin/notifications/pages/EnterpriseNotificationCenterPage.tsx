...
export const EnterpriseNotificationCenterPage: React.FC = () => {
    ...
    try {
      const [statsRes, actRes, notifRes] = await Promise.all([
        ...
        enterpriseNotificationService.getActivities({
          module: activeSection === 'auth-logs' ? 'AUTH' : activeSection === 'bookings' ? 'BOOKING' : activeSection === 'payments' ? 'PAYMENT' : activeSection === 'cms' ? 'BANNER' : moduleFilter,
      ...
      if (notifRes.data?.success) setNotificationsList(notifRes.data.data || []);
    } catch (err) {
  ...
  const handleSeed = async () => {
    try {
      ...
      addNotification('Telemetry Initialized', 'Real-time telemetry and sample events seeded.', 'success');
      fetchCenterData();
    } catch (err) {
      addNotification('Seed Error', getErrorMessage(err, 'Unable to seed telemetry.'), 'error');
    }
  };
  ...
  const handleMarkRead = async (id: string) => {
    try {
      ...
      addNotification('Action Error', getErrorMessage(err, 'Could not mark read.'), 'error');
    }
  };
  ...
  const handleDeleteNotif = async (id: string) => {
    try {
      ...
      addNotification('Delete Error', getErrorMessage(err, 'Could not delete.'), 'error');
    }
  };
  ...
  const handleBulkAction = async (action: 'delete' | 'mark_read') => {
    ...
    try {
      ...
      addNotification('Bulk Action Complete', `${selectedIds.length} items updated.`, 'success');
      setSelectedIds([]);
      fetchCenterData();
    } catch (err) {
      addNotification('Bulk Error', getErrorMessage(err, 'Bulk operation failed.'), 'error');
    }
  };

  const handleExportCSV = () => {
    const exportData = activeSection === 'activities' || activeSection === 'timeline' ? activities : notificationsList;
    if (exportData.length === 0) return;

    const headers = 'ID,Module,Action,Description,Severity,Timestamp\n';
    const rows = exportData
      .map((x) => `"${x._id || x.activityId}","${x.module || ''}","${x.action || ''}","${(x.description || x.message || '').replace(/"/g, '""')}","${x.severity || 'info'}","${x.createdAt || x.timestamp || ''}"`)
      .join('\n');
    ...
    link.href = url;
    link.setAttribute('download', `tirvona_telemetry_${activeSection}_${Date.now()}.csv`);
  ...
  const getSeverityBadge = (severity: string) => {
    switch (severity?.toLowerCase()) {
      ...
      case 'emergency':
      case 'security':
        return <span className="px-2.5 py-0.5 bg-rose-100 dark:bg-rose-950/60 text-rose-800 dark:text-rose-300 rounded-full text-[9px] font-black uppercase tracking-wider">🚨 {severity}</span>;
      case 'warning':
        return <span className="px-2.5 py-0.5 bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 rounded-full text-[9px] font-black uppercase tracking-wider">⚠️ Warning</span>;
      case 'success':
        return <span className="px-2.5 py-0.5 bg-emerald-100 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-300 rounded-full text-[9px] font-black uppercase tracking-wider">✅ Success</span>;
      default:
        return <span className="px-2.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 rounded-full text-[9px] font-bold uppercase tracking-wider">ℹ️ Info</span>;
    }
  };
    ...
    <div className="space-y-6 text-left max-w-7xl mx-auto pb-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 font-semibold mt-0.5">
              Real-time platform telemetry, authentication logs, security events, and live Socket.IO feeds.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 p-4 rounded-2xl shadow-sm">
          <span className="text-[10px] text-gray-400 font-bold uppercase block">Today's Notifications</span>
          <span className="text-2xl font-black text-[#0B192C] dark:text-white mt-1 block">{stats?.todaysNotifications ?? 0}</span>
        </div>
        <div className="bg-white dark:bg-[#0B192C] border border-rose-200 dark:border-rose-950/60 p-4 rounded-2xl shadow-sm">
          <span className="text-[10px] text-rose-500 font-bold uppercase block">Critical Alerts</span>
          <span className="text-2xl font-black text-rose-600 dark:text-rose-400 mt-1 block">{stats?.criticalAlerts ?? 0}</span>
        </div>
        <div className="bg-white dark:bg-[#0B192C] border border-amber-200 dark:border-amber-950/60 p-4 rounded-2xl shadow-sm">
          <span className="text-[10px] text-amber-500 font-bold uppercase block">Unread Notifications</span>
          <span className="text-2xl font-black text-amber-600 dark:text-amber-400 mt-1 block">{stats?.unreadNotifications ?? 0}</span>
        </div>
        <div className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 p-4 rounded-2xl shadow-sm">
          <span className="text-[10px] text-gray-400 font-bold uppercase block">Failed Logins</span>
          <span className="text-2xl font-black text-gray-800 dark:text-gray-200 mt-1 block">{stats?.failedLogins ?? 0}</span>
        </div>
        <div className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 p-4 rounded-2xl shadow-sm">
          <span className="text-[10px] text-gray-400 font-bold uppercase block">Failed Payments</span>
          <span className="text-2xl font-black text-gray-800 dark:text-gray-200 mt-1 block">{stats?.failedPayments ?? 0}</span>
        </div>
        <div className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 p-4 rounded-2xl shadow-sm">
          <span className="text-[10px] text-gray-400 font-bold uppercase block">Server / API Errors</span>
          <span className="text-2xl font-black text-gray-800 dark:text-gray-200 mt-1 block">{stats?.apiErrors ?? 0}</span>
        </div>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none border-b border-gray-200 dark:border-slate-800">
        {[
          ...
          <button
            key={tab.key}
            onClick={() => navigate(`/admin/enterprise-notifications/${tab.key}`)}
      ...
      {viewMode === 'timeline' || activeSection === 'timeline' ? (
        ...
        <div className="bg-white dark:bg-[#0B192C] border border-gray-100 dark:border-slate-800 rounded-[28px] p-6 shadow-sm space-y-6">
          <h3 className="font-extrabold text-base text-[#0B192C] dark:text-white flex items-center gap-2">
            <Clock size={18} className="text-[#0A4DA6]" /> Live Chronological System Timeline
          </h3>
              ...
              <div key={item._id || idx} className="relative group">
                ...
                <div className="bg-gray-50 dark:bg-slate-900/60 p-4 rounded-2xl border border-gray-100 dark:border-slate-800 space-y-2">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-xs text-[#0B192C] dark:text-white">{item.action}</span>
                      {getSeverityBadge(item.severity)}
                    </div>
                    <span className="text-[10px] text-gray-400 font-semibold">{new Date(item.timestamp || item.createdAt).toLocaleString()}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-4 text-[10px] text-gray-400 pt-1 font-mono">
                    <span>User: {item.userName} ({item.role})</span>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                {activities.map((item, idx) => (
                  <tr key={item._id || idx} className="hover:bg-gray-50/50 dark:hover:bg-slate-900/40">
                    <td className="py-4 px-6 font-mono text-gray-400 whitespace-nowrap">
                      {new Date(item.timestamp || item.createdAt).toLocaleString()}
                    </td>
                    <td className="py-4 px-6">
                      <div className="flex flex-col">
                        <span className="font-extrabold text-[#0B192C] dark:text-white">{item.action}</span>
                        <span className="text-[10px] text-[#0A4DA6] font-bold uppercase">{item.module}</span>
                      </div>
                    </td>
                    <td className="py-4 px-6">
                      <div className="flex flex-col">
                        <span className="font-semibold text-gray-800 dark:text-gray-200">{item.userName}</span>
                        <span className="text-[10px] text-gray-400">{item.role} • {item.ipAddress}</span>
                      </div>
                    </td>
                    <td className="py-4 px-6 font-medium text-gray-600 dark:text-gray-300 max-w-xs truncate">
                      {item.description}
                    </td>
                    <td className="py-4 px-6">{getSeverityBadge(item.severity)}</td>
