...
export class AartiBookingService {
  ...
  /// Confirms an aarti-pass booking's payment from a verified Razorpay
  /// webhook event, independent of any client callback — see the equivalent
  /// method on `BookingsService` for the full rationale (low-RAM devices
  /// losing the in-app callback when Android kills the process during a
  /// UPI-app redirect). Returns `false` when this module doesn't own the
  /// order.
